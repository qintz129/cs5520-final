            ...
            (async () => { 
              ...
              if (hasNewImage && uploadUri) {
                try {
                  ...
                  if (initialPassword === password) {
                    await updateToDB(auth.currentUser.uid, "users", null, null, { name:name, image: newImageUri }); 
                  } 
                  else {
                    await updateToDB(auth.currentUser.uid, "users", null, null, { name:name, password:password, image: newImageUri }); 
                  } 
                } catch (error) {
                  console.error("Error updating user info with new image:", error);
                }
              } else { 
                if (initialPassword === password) {
                  await updateToDB(auth.currentUser.uid, "users", null, null, { name:name }); 
                } 
                else {
                  await updateToDB(auth.currentUser.uid, "users", null, null, { name:name, password:password }); 
                } 
              }
